"""Build and verify Thin Fork post-disposition qualification evidence."""
...
BASE_PLATFORM_COMMIT = "8a56132dedf3e5b75170431bdeb3e2b8c0812997"
...
RUNNER_RELATIVE = "pmorg/scripts/run_thin_fork_post_disposition_suite.py"
BUILD_SCRIPT_RELATIVE = (
    "pmorg/scripts/build_thin_fork_post_disposition_qualification.py"
)
APPLICATION_RELATIVE = (
    "backend/pmorg/application/thin_fork_post_disposition_qualification.py"
)
INDEX_RELATIVE = "pmorg/capabilities/thin-fork-post-disposition-qualification-v1.json"
INDEX_SCHEMA_RELATIVE = (
    "pmorg/capabilities/thin-fork-post-disposition-qualification-v1.schema.json"
)
MANIFEST_RELATIVE = (
    "pmorg/capabilities/thin-fork-post-disposition-test-manifest-v1.json"
)
MANIFEST_SCHEMA_RELATIVE = (
    "pmorg/capabilities/thin-fork-post-disposition-test-manifest-v1.schema.json"
)
RESULT_SCHEMA_RELATIVE = (
    "pmorg/capabilities/thin-fork-post-disposition-test-result-v1.schema.json"
)
SUITE_RECEIPT_RELATIVE = (
    "pmorg/capabilities/thin-fork-post-disposition-suite-receipt-v1.json"
)
SUITE_RECEIPT_SCHEMA_RELATIVE = (
    "pmorg/capabilities/thin-fork-post-disposition-suite-receipt-v1.schema.json"
)
REPORT_RELATIVE = (
    "pmorg/capabilities/thin-fork-post-disposition-qualification-report-v1.json"
)
RESULT_ROOT_RELATIVE = "pmorg/capabilities/thin-fork-post-disposition-test-results"
...
REQUIREMENT_IDS = ["A-PATCH-001", "PLT-004"]
LEDGER_IDS = ["PL-000"]
TEST_COUNT = 87
INDEX_VERSION = "1.0.0"
CLAIM_BOUNDARY = "thin_fork_post_disposition_qualification_only_no_disposition"


class ThinForkPostDispositionQualificationError(ValueError):
    """Raised when Thin Fork PDQ evidence is incomplete or drifted."""
...
def _safe_path(repository_root: Path, relative_path: str) -> Path:
    ...
    try:
        ...
        raise ThinForkPostDispositionQualificationError(
...
def _verified_payload(
    ...
    if not isinstance(relative_path, str) or not relative_path:
        raise ThinForkPostDispositionQualificationError(f"{label} has no relative path")
    payload = _safe_path(repository_root, relative_path).read_bytes()
    if reference.get("digest") != sha256_digest(payload):
        raise ThinForkPostDispositionQualificationError(f"{label} digest drifted")
    if reference.get("size_bytes") != len(payload):
        raise ThinForkPostDispositionQualificationError(f"{label} size drifted")
    return payload
...
def _git_blob(repository_root: Path, relative_path: str) -> bytes:
    try:
        ...
        raise ThinForkPostDispositionQualificationError(
            f"trusted Thin Fork path is absent from base: {relative_path}"
        ) from error
...
def _assert_trust_boundary_immutable(repository_root: Path) -> None:
    for relative_path in (VERIFY_FORK_RELATIVE, VERIFY_FORK_TEST_RELATIVE):
        if _safe_path(repository_root, relative_path).read_bytes() != _git_blob(
            ...
            raise ThinForkPostDispositionQualificationError(
                f"trusted Thin Fork path changed in PDQ slice: {relative_path}"
            )
...
def _ledger_entries(repository_root: Path) -> list[dict[str, Any]]:
    ...
    if any(item not in by_id for item in LEDGER_IDS):
        raise ThinForkPostDispositionQualificationError(
            "Thin Fork patch-ledger ownership is missing"
        )
...
def _run_runner(
    ...
    if list_only:
        ...
        if not isinstance(value, list) or not all(
            ...
            raise ThinForkPostDispositionQualificationError(
                "Thin Fork runner emitted an invalid test list"
            )
    ...
    if not isinstance(value, dict):
        raise ThinForkPostDispositionQualificationError(
            "Thin Fork runner emitted an invalid suite receipt"
        )
...
def _manifest_document(
    ...
    implementation_bindings = [
        _binding(repository_root, VERIFY_FORK_RELATIVE),
        _binding(repository_root, VERIFY_FORK_TEST_RELATIVE),
    ]
    ledger_entries = _ledger_entries(repository_root)
    return {
        "schema_version": "pmorg.thin-fork-post-disposition-test-manifest/v1",
...
def manifest_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "$id": "urn:pmorg:thin-fork-post-disposition-test-manifest:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.thin-fork-post-disposition-test-manifest/v1"
            },
...
def suite_receipt_schema() -> dict[str, Any]:
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "urn:pmorg:thin-fork-post-disposition-suite-receipt:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.thin-fork-post-disposition-suite-receipt/v1"
            },
            "suite": {"const": "pmorg.tests.test_verify_fork"},
...
def result_schema() -> dict[str, Any]:
    ...
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "urn:pmorg:thin-fork-post-disposition-test-result:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.thin-fork-post-disposition-test-result/v1"
            },
            "capability_id": {"const": CAPABILITY_ID},
            "test_id": {"type": "string", "minLength": 1},
            "fully_qualified_name": {"type": "string", "minLength": 1},
...
def index_schema() -> dict[str, Any]:
    ...
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "urn:pmorg:thin-fork-post-disposition-qualification-index:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.thin-fork-post-disposition-qualification-index/v1"
            },
...
def build_thin_fork_post_disposition_qualification(
    ...
    if len(identities) != TEST_COUNT:
        raise ThinForkPostDispositionQualificationError(
            "Thin Fork PDQ test denominator drifted"
        )
    ...
    for item in cast(list[dict[str, Any]], receipt["tests"]):
        ...
        test_id = _test_id(identity)
        result = {
            "schema_version": "pmorg.thin-fork-post-disposition-test-result/v1",
            "capability_id": CAPABILITY_ID,
            "test_id": test_id,
            "fully_qualified_name": identity,
    ...
    try:
        ...
        raise ThinForkPostDispositionQualificationError(
            "Thin Fork PDQ report failed the contract model"
        ) from error
    ...
    if errors:
        raise ThinForkPostDispositionQualificationError(
            f"Thin Fork PDQ report schema failed: {errors[0]}"
        )
    ...
    index = {
        "schema_version": ("pmorg.thin-fork-post-disposition-qualification-index/v1"),
    ...
    if errors:
        raise ThinForkPostDispositionQualificationError(
            f"Thin Fork PDQ index schema failed: {errors[0]}"
        )
    outputs[INDEX_RELATIVE] = canonical_document_bytes(index)
    return outputs


def write_thin_fork_post_disposition_qualification(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    outputs = build_thin_fork_post_disposition_qualification(repository_root)
...
def check_thin_fork_post_disposition_qualification(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    _assert_trust_boundary_immutable(repository_root)
    ...
    if manifest != _manifest_document(repository_root, identities):
        raise ThinForkPostDispositionQualificationError(
            "Thin Fork PDQ manifest derivation drifted"
        )
    receipt = json.loads(
        _verified_payload(
    ...
    if [item["fully_qualified_name"] for item in receipt["tests"]] != identities:
        raise ThinForkPostDispositionQualificationError(
            "Thin Fork PDQ receipt denominator drifted"
        )
    runtime_measurement, runtime_digest = _runtime_measurement(repository_root, receipt)
    if index["runtime_identity_digests"] != [runtime_digest]:
        raise ThinForkPostDispositionQualificationError(
            "Thin Fork PDQ runtime identity drifted"
        )
    ...
    try:
        ...
        raise ThinForkPostDispositionQualificationError(
            "committed Thin Fork PDQ report failed the contract model"
        ) from error
    ...
    if len(result_refs) != TEST_COUNT or len(evidence) != TEST_COUNT:
        raise ThinForkPostDispositionQualificationError(
            "Thin Fork PDQ result coverage drifted"
        )
    ...
    for identity, reference, item in zip(
        ...
        if (
            ...
            raise ThinForkPostDispositionQualificationError(
                f"Thin Fork PDQ evidence drifted: {identity}"
            )
    ...
    if (
        ...
        raise ThinForkPostDispositionQualificationError(
            "Thin Fork PDQ report counters or bindings drifted"
        )
    committed_paths = {
    ...
    if committed_paths != observed_paths:
        raise ThinForkPostDispositionQualificationError(
            "Thin Fork PDQ result directory is not byte-closed"
        )
...
__all__ = [
    "ThinForkPostDispositionQualificationError",
    "build_thin_fork_post_disposition_qualification",
    "check_thin_fork_post_disposition_qualification",
    "index_schema",
    "manifest_schema",
    "result_schema",
    "suite_receipt_schema",
    "write_thin_fork_post_disposition_qualification",
]
