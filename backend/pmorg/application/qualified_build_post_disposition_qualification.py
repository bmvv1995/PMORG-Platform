"""Build and verify Qualified Build post-disposition qualification evidence."""
...
BUILD_CE_ARTIFACT_RELATIVE = "pmorg/scripts/build_ce_artifact.py"
VERIFY_CE_BUILD_RELATIVE = "pmorg/scripts/verify_ce_build.py"
ENGINE_RELATIVE = "pmorg/build/artifact.py"
TEST_MODULE_RELATIVES = (
    "pmorg/tests/test_ce_build_substrate.py",
    "pmorg/tests/test_ce_image_contexts.py",
)
TEST_SUITE_NAMES = (
    "pmorg.tests.test_ce_build_substrate",
    "pmorg.tests.test_ce_image_contexts",
)
RUNNER_RELATIVE = "pmorg/scripts/run_qualified_build_post_disposition_suite.py"
BUILD_SCRIPT_RELATIVE = (
    "pmorg/scripts/build_qualified_build_post_disposition_qualification.py"
)
APPLICATION_RELATIVE = (
    "backend/pmorg/application/qualified_build_post_disposition_qualification.py"
)
INDEX_RELATIVE = (
    "pmorg/capabilities/qualified-build-post-disposition-qualification-v1.json"
)
INDEX_SCHEMA_RELATIVE = (
    "pmorg/capabilities/qualified-build-post-disposition-qualification-v1.schema.json"
)
MANIFEST_RELATIVE = (
    "pmorg/capabilities/qualified-build-post-disposition-test-manifest-v1.json"
)
MANIFEST_SCHEMA_RELATIVE = (
    "pmorg/capabilities/qualified-build-post-disposition-test-manifest-v1.schema.json"
)
RESULT_SCHEMA_RELATIVE = (
    "pmorg/capabilities/qualified-build-post-disposition-test-result-v1.schema.json"
)
SUITE_RECEIPT_RELATIVE = (
    "pmorg/capabilities/qualified-build-post-disposition-suite-receipt-v1.json"
)
SUITE_RECEIPT_SCHEMA_RELATIVE = (
    "pmorg/capabilities/qualified-build-post-disposition-suite-receipt-v1.schema.json"
)
REPORT_RELATIVE = (
    "pmorg/capabilities/qualified-build-post-disposition-qualification-report-v1.json"
)
RESULT_ROOT_RELATIVE = (
    "pmorg/capabilities/qualified-build-post-disposition-test-results"
)
...
TEST_COUNT = 12
INDEX_VERSION = "1.0.0"
CLAIM_BOUNDARY = "qualified_build_post_disposition_qualification_only_no_disposition"
...
ALLOWED_SLICE_PATHS = {
    ...
    SUITE_RECEIPT_RELATIVE,
    SUITE_RECEIPT_SCHEMA_RELATIVE,
    "backend/pmorg/tests/test_qualified_build_post_disposition_qualification.py",
}
...
class QualifiedBuildPostDispositionQualificationError(ValueError):
    """Raised when Qualified Build PDQ evidence is incomplete or drifted."""
...
def _safe_path(repository_root: Path, relative_path: str) -> Path:
    ...
    try:
        ...
        raise QualifiedBuildPostDispositionQualificationError(
...
def _verified_payload(
    ...
    if reference.get("digest") != sha256_digest(payload):
        raise QualifiedBuildPostDispositionQualificationError(f"{label} digest drifted")
    if reference.get("size_bytes") != len(payload):
        raise QualifiedBuildPostDispositionQualificationError(f"{label} size drifted")
    return payload
...
def _historical_blob(
    ...
    try:
        ...
        raise QualifiedBuildPostDispositionQualificationError(
            f"Qualified Build historical path is absent from {commit}: {relative_path}"
        ) from error
...
def _assert_trust_boundary_immutable(repository_root: Path) -> None:
    for relative_path in (
        BUILD_CE_ARTIFACT_RELATIVE,
        VERIFY_CE_BUILD_RELATIVE,
        ENGINE_RELATIVE,
        *TEST_MODULE_RELATIVES,
    ):
        if _safe_path(repository_root, relative_path).read_bytes() != _git_blob(
            ...
            raise QualifiedBuildPostDispositionQualificationError(
                f"trusted Qualified Build path changed in PDQ slice: {relative_path}"
            )


def _historical_result_paths(repository_root: Path) -> set[str]:
    if Q6D_EVIDENCE_COMMIT is None:
        return set()
    try:
        ...
        raise QualifiedBuildPostDispositionQualificationError(
            "cannot enumerate historical Q6d result paths"
        ) from error
...
def _assert_archival_evidence_immutable(repository_root: Path) -> None:
    ...
    for relative_path in sorted(historical_paths):
        try:
            ...
            raise QualifiedBuildPostDispositionQualificationError(
                f"Q6d archival evidence is missing: {relative_path}"
            ) from error
        historical = _historical_blob(
            repository_root, relative_path, commit=Q6D_EVIDENCE_COMMIT
        )
        if live != historical:
            raise QualifiedBuildPostDispositionQualificationError(
                f"Q6d archival evidence drifted: {relative_path}"
            )
...
def _assert_append_only_sequence(*, base: Any, live: Any, label: str) -> None:
    if not isinstance(base, list) or not isinstance(live, list):
        raise QualifiedBuildPostDispositionQualificationError(
            f"Q6d patch ledger {label} is not a list"
        )
    if len(live) < len(base) or live[: len(base)] != base:
        raise QualifiedBuildPostDispositionQualificationError(
            f"Q6d patch ledger {label} history is not an exact prefix"
        )
    identifiers: list[str] = []
    for item in live:
        if not isinstance(item, dict) or not isinstance(item.get("id"), str):
            raise QualifiedBuildPostDispositionQualificationError(
                f"Q6d patch ledger {label} contains an invalid ID"
            )
        identifiers.append(cast(str, item["id"]))
    if len(identifiers) != len(set(identifiers)):
        raise QualifiedBuildPostDispositionQualificationError(
            f"Q6d patch ledger {label} IDs are not unique"
        )
...
def _assert_patch_ledger_append_only(repository_root: Path) -> None:
    ...
    try:
        ...
        raise QualifiedBuildPostDispositionQualificationError(
            "Q6d patch ledger cannot be read as JSON"
        ) from error
    if not isinstance(base, dict) or not isinstance(live, dict):
        raise QualifiedBuildPostDispositionQualificationError(
            "Q6d patch ledger is not an object"
        )
    if set(live) != set(base):
        raise QualifiedBuildPostDispositionQualificationError(
            "Q6d patch ledger top-level structure drifted"
        )
    for key in LEDGER_TOP_LEVEL_PINS:
        if live.get(key) != base.get(key):
            raise QualifiedBuildPostDispositionQualificationError(
                f"Q6d patch ledger top-level pin drifted: {key}"
            )
...
def _assert_historical_integrity(repository_root: Path) -> None:
    """Bind only Q6d evidence, static trust files, and append-only registries."""
...
def _ledger_entries(repository_root: Path) -> list[dict[str, Any]]:
    ...
    if any(item not in by_id for item in LEDGER_IDS):
        raise QualifiedBuildPostDispositionQualificationError(
            "Qualified Build patch-ledger ownership is missing"
        )
...
def _run_runner(
    ...
    if list_only:
        ...
        if not isinstance(value, list) or not all(
            ...
            raise QualifiedBuildPostDispositionQualificationError(
                "Qualified Build runner emitted an invalid test list"
            )
    ...
    if not isinstance(value, dict):
        raise QualifiedBuildPostDispositionQualificationError(
            "Qualified Build runner emitted an invalid suite receipt"
        )
...
def _manifest_document(
    ...
    implementation_bindings = [
        _binding(repository_root, BUILD_CE_ARTIFACT_RELATIVE),
        _binding(repository_root, VERIFY_CE_BUILD_RELATIVE),
    ]
    ...
    return {
        "schema_version": "pmorg.qualified-build-post-disposition-test-manifest/v1",
...
def manifest_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "$id": "urn:pmorg:qualified-build-post-disposition-test-manifest:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.qualified-build-post-disposition-test-manifest/v1"
            },
...
def suite_receipt_schema() -> dict[str, Any]:
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "urn:pmorg:qualified-build-post-disposition-suite-receipt:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.qualified-build-post-disposition-suite-receipt/v1"
            },
            "pmorg_platform_commit": {"const": BASE_PLATFORM_COMMIT},
            "suites": {
                "const": [
...
def result_schema() -> dict[str, Any]:
    ...
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "urn:pmorg:qualified-build-post-disposition-test-result:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.qualified-build-post-disposition-test-result/v1"
            },
            "capability_id": {"const": CAPABILITY_ID},
            "test_id": {"type": "string", "minLength": 1},
            "fully_qualified_name": {"type": "string", "minLength": 1},
...
def index_schema() -> dict[str, Any]:
    ...
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "urn:pmorg:qualified-build-post-disposition-qualification-index:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.qualified-build-post-disposition-qualification-index/v1"
            },
...
def build_qualified_build_post_disposition_qualification(
    ...
    if len(identities) != TEST_COUNT:
        raise QualifiedBuildPostDispositionQualificationError(
            "Qualified Build PDQ test denominator drifted"
        )
    ...
    for item in cast(list[dict[str, Any]], receipt["tests"]):
        ...
        test_id = _test_id(identity)
        result = {
            "schema_version": "pmorg.qualified-build-post-disposition-test-result/v1",
            "capability_id": CAPABILITY_ID,
            "test_id": test_id,
            "fully_qualified_name": identity,
    ...
    try:
        ...
        raise QualifiedBuildPostDispositionQualificationError(
            "Qualified Build PDQ report failed the contract model"
        ) from error
    ...
    if errors:
        raise QualifiedBuildPostDispositionQualificationError(
            f"Qualified Build PDQ report schema failed: {errors[0]}"
        )
    ...
    index = {
        "schema_version": (
            "pmorg.qualified-build-post-disposition-qualification-index/v1"
        ),
    ...
    if errors:
        raise QualifiedBuildPostDispositionQualificationError(
            f"Qualified Build PDQ index schema failed: {errors[0]}"
        )
    outputs[INDEX_RELATIVE] = canonical_document_bytes(index)
    return outputs


def write_qualified_build_post_disposition_qualification(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    outputs = build_qualified_build_post_disposition_qualification(repository_root)
...
def check_qualified_build_post_disposition_qualification(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    _assert_trust_boundary_immutable(repository_root)
    ...
    if manifest != _manifest_document(repository_root, identities):
        raise QualifiedBuildPostDispositionQualificationError(
            "Qualified Build PDQ manifest derivation drifted"
        )
    receipt = json.loads(
        _verified_payload(
    ...
    if [item["fully_qualified_name"] for item in receipt["tests"]] != identities:
        raise QualifiedBuildPostDispositionQualificationError(
            "Qualified Build PDQ receipt denominator drifted"
        )
    runtime_measurement, runtime_digest = _runtime_measurement(repository_root, receipt)
    if index["runtime_identity_digests"] != [runtime_digest]:
        raise QualifiedBuildPostDispositionQualificationError(
            "Qualified Build PDQ runtime identity drifted"
        )
    ...
    try:
        ...
        raise QualifiedBuildPostDispositionQualificationError(
            "committed Qualified Build PDQ report failed the contract model"
        ) from error
    ...
    if len(result_refs) != TEST_COUNT or len(evidence) != TEST_COUNT:
        raise QualifiedBuildPostDispositionQualificationError(
            "Qualified Build PDQ result coverage drifted"
        )
    ...
    for identity, reference, item in zip(
        ...
        if (
            ...
            raise QualifiedBuildPostDispositionQualificationError(
                f"Qualified Build PDQ evidence drifted: {identity}"
            )
    ...
    if (
        ...
        raise QualifiedBuildPostDispositionQualificationError(
            "Qualified Build PDQ report counters or bindings drifted"
        )
    committed_paths = {
    ...
    for reference in cast(list[dict[str, Any]], index["derivation_artifacts"]):
        ...
        if relative_path in (APPLICATION_RELATIVE, ENGINE_RELATIVE):
            ...
            if reference.get("digest") != sha256_digest(payload) or reference.get(
                ...
                raise QualifiedBuildPostDispositionQualificationError(
                    "historical Q6d derivation reference drifted"
                )
        else:
            _verified_payload(
                repository_root, reference, label="PDQ derivation artifact"
...
__all__ = [
    "QualifiedBuildPostDispositionQualificationError",
    "build_qualified_build_post_disposition_qualification",
    "check_qualified_build_post_disposition_qualification",
    "index_schema",
    "manifest_schema",
    "result_schema",
    "suite_receipt_schema",
    "write_qualified_build_post_disposition_qualification",
]
