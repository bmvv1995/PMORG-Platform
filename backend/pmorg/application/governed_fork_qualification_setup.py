"""Build unactivated governed-fork vectors and exhaustive screening evidence."""
...
EXPECTED_CANDIDATES = 67
EXPECTED_MEMBERSHIPS = 5364
...
SEARCH_EVIDENCE_RELATIVE = (
    "pmorg/capabilities/candidate-search/governed-onyx-fork-search-evidence-v1.json"
)
HIT_CLASSIFICATION_RELATIVE = (
    "pmorg/capabilities/candidate-search/governed-onyx-fork-hit-classification-v1.json"
)
...
VECTOR_RELATIVES = {
    test_id: (
        ...
        f"governed-onyx-fork-{test_id}-v1.json"
    )
    for test_id in TEST_IDS
}
EVIDENCE_RELATIVE = "pmorg/capabilities/governed-fork-interface-fit-evidence-v1.json"
EVIDENCE_SCHEMA_RELATIVE = (
    "pmorg/capabilities/governed-fork-interface-fit-evidence-v1.schema.json"
)
DERIVATION_RELATIVES = (
    "backend/pmorg/application/governed_fork_qualification_setup.py",
    "pmorg/scripts/build_governed_fork_qualification_setup.py",
)
RUNTIME_ARTIFACTS = (
...
_SIGNALS = {
    "fork_identity": ("base", "branch", "commit", "fork", "revision", "tree"),
    "ownership_and_seams": ("allowlist", "owner", "ownership", "seam"),
    "surface_policy": ("ce", "ee", "license", "licensed", "surface"),
...
class GovernedForkQualificationSetupError(ValueError):
    """Raised when Q5a evidence cannot be reproduced exactly."""
...
def _safe_path(repository_root: Path, relative_path: str) -> Path:
    ...
    try:
        ...
        raise GovernedForkQualificationSetupError(
...
_TEST_SEMANTICS = {
    "A-FORK-001": (
        "present a candidate implementation that enforces exact trusted-base, candidate-tree and protected-base identities",
        "accept the conforming injected implementation and return an empty ordered violation set",
        "mutate a pinned identity or permit an unknown ownership route",
        "reject fail-closed and cite the exact identity or ownership violation",
    ),
    "A-SURFACE-001": (
        "present a candidate implementation that enforces the declared CE/EE surface and immutable surface policy",
        "accept the conforming injected implementation and return an empty ordered violation set",
        "mutate a declared surface boundary or admit a hybrid surface state",
        "reject fail-closed and cite the exact surface-policy violation",
    ),
    "A-UPSTREAM-001": (
        "present a candidate implementation that enforces pinned upstream content and complete patch-ledger governance",
        "accept the conforming injected implementation and return an empty ordered violation set",
        "mutate upstream bytes without a complete governed patch route",
        "reject fail-closed and cite the exact upstream or ledger violation",
    ),
}
...
def _build_vectors(repository_root: Path) -> tuple[dict[str, bytes], dict[str, Any]]:
    ...
    manifest = {
        ...
        "extension_id": "governed-onyx-fork-v1",
...
def evidence_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "$id": "urn:pmorg:governed-fork-interface-fit-evidence:v1",
        "title": "PMORG exhaustive governed-fork interface-fit screening evidence",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.governed-fork-interface-fit-evidence/v1"
            },
            ...
            "selection_policy": {
                "const": "all_67_candidates_from_governed_onyx_fork_hit_classification"
            },
            "falsifiability_contract": {
                "type": "object",
                "additionalProperties": False,
...
def _build_screening(
    ...
    if classification.get(
        ...
        raise GovernedForkQualificationSetupError(
            "governed-fork classification denominator drifted"
        )
    selected = [
        candidate
    ...
    if actual_ids != expected_ids or len(actual_ids) != EXPECTED_CANDIDATES:
        raise GovernedForkQualificationSetupError(
            "candidate inputs do not equal the committed 67-candidate denominator"
        )
    ...
    if memberships != EXPECTED_MEMBERSHIPS or scan_memberships != EXPECTED_MEMBERSHIPS:
        raise GovernedForkQualificationSetupError(
            "governed-fork projected membership denominator drifted"
        )
    vector_refs = [
        {
    ...
    document = {
        "schema_version": "pmorg.governed-fork-interface-fit-evidence/v1",
        ...
        "selection_policy": "all_67_candidates_from_governed_onyx_fork_hit_classification",
    ...
    if errors:
        raise GovernedForkQualificationSetupError(
            f"governed-fork screening schema violation: {errors[0]}"
        )
    return {
...
def build_governed_fork_qualification_setup(
...
def write_governed_fork_qualification_setup(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    for relative_path, payload in build_governed_fork_qualification_setup(
        repository_root
    ).items():
...
def check_governed_fork_qualification_setup(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    for relative_path, payload in build_governed_fork_qualification_setup(
        ...
        try:
            ...
            raise GovernedForkQualificationSetupError(
                f"committed Q5a artifact is missing: {relative_path}"
            ) from error
        if actual != payload:
            raise GovernedForkQualificationSetupError(
                f"committed Q5a artifact drifted: {relative_path}"
            )
