"""Build unactivated capability-disposition-qualification vectors and exhaustive screening evidence."""
...
CONTRACT_TEST_MANIFEST_RELATIVE = (
...
IMPLEMENTATION_BY_TEST = {
    "A-PATCH-002": "backend/pmorg/application/qualification_oracles.py",
    "A-PATCH-003": "backend/pmorg/application/qualification_oracles.py",
    "A-PATCH-004": "backend/pmorg/application/qualification.py",
    "A-PATCH-005": "backend/pmorg/application/qualification_oracles.py",
    "A-PATCH-006": "backend/pmorg/application/qualification.py",
}
CANDIDATE_INPUTS_RELATIVE = "pmorg/capabilities/candidate-inputs-v1.json"
SEARCH_EVIDENCE_RELATIVE = "pmorg/capabilities/candidate-search/capability-disposition-qualification-search-evidence-v1.json"
HIT_CLASSIFICATION_RELATIVE = "pmorg/capabilities/candidate-search/capability-disposition-qualification-hit-classification-v1.json"
VECTOR_MANIFEST_RELATIVE = "pmorg/capabilities/qualification-test-vector-extension-capability-disposition-qualification-v1.json"
VECTOR_RELATIVES = {
    test_id: (
        "pmorg/capabilities/qualification-test-vector-extensions/"
        f"capability-disposition-qualification-{test_id}-v1.json"
    )
    for test_id in TEST_IDS
}
EVIDENCE_RELATIVE = "pmorg/capabilities/capability-disposition-qualification-interface-fit-evidence-v1.json"
EVIDENCE_SCHEMA_RELATIVE = "pmorg/capabilities/capability-disposition-qualification-interface-fit-evidence-v1.schema.json"
...
_SIGNALS = {
    "catalog_binding": ("capability", "catalog", "contract", "requirement"),
    "candidate_binding": ("candidate", "classification", "manifest", "search"),
    "evidence_binding": ("attestation", "digest", "dsse", "evidence", "sha256"),
    "qualification_binding": ("oracle", "qualification", "verdict", "verify"),
    "patch_binding": ("ledger", "patch", "scope", "upstream"),
...
class CapabilityDispositionQualificationSetupError(ValueError):
    """Raised when Q7a evidence cannot be reproduced exactly."""
...
def _safe_path(repository_root: Path, relative_path: str) -> Path:
    ...
    try:
        ...
        raise CapabilityDispositionQualificationSetupError(
...
_TEST_SEMANTICS = {
    "A-PATCH-002": (
        "present projected candidate bytes whose capability catalog and exact contract-test bindings remain internally consistent",
        "accept the conforming injected projection with every catalog and contract identity preserved",
        "mutate a catalog capability, requirement or contract-test identity in the projected bytes",
        "reject fail-closed and cite the exact catalog or contract binding inconsistency",
    ),
    "A-PATCH-003": (
        "present projected candidate bytes whose source scope, exhaustive search and candidate identities remain byte-closed",
        "accept the conforming injected projection with the complete search denominator preserved",
        "remove, replace or reclassify a source-scope or candidate-search binding in the projected bytes",
        "reject fail-closed and cite the exact scope, search or candidate identity drift",
    ),
    "A-PATCH-004": (
        "present projected candidate bytes whose qualification evidence, canonical payload and signature bindings are complete",
        "accept the conforming injected projection with all qualification identities content-addressed",
        "mutate a qualification payload, evidence reference or signature binding in the projected bytes",
        "reject fail-closed and cite the exact qualification evidence inconsistency",
    ),
    "A-PATCH-005": (
        "present projected candidate bytes whose CE build verification and disposition inputs remain exactly bound",
        "accept the conforming injected projection with all build-verification identities preserved",
        "detach or alter a CE build verification or disposition input binding in the projected bytes",
        "reject fail-closed and cite the exact build-verification or disposition-input drift",
    ),
    "A-PATCH-006": (
        "present projected candidate bytes satisfying the combined scope, search and qualification invariant set",
        "accept the conforming injected projection only when every combined invariant remains true",
        "mutate one scope, search or qualification invariant binding in the projected bytes",
        "reject fail-closed and cite the exact combined-invariant violation",
    ),
}
...
def _build_vectors(repository_root: Path) -> tuple[dict[str, bytes], dict[str, Any]]:
    ...
    expected_contract_manifest = _artifact_ref(
        repository_root,
        CONTRACT_TEST_MANIFEST_RELATIVE,
        media_type="application/json",
    )
    if interface.get("contract_test_manifest") != expected_contract_manifest:
        raise CapabilityDispositionQualificationSetupError(
            "qualification interface contract-test manifest drifted"
        )
...
def evidence_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "$id": "urn:pmorg:capability-disposition-qualification-interface-fit-evidence:v1",
        "title": "PMORG exhaustive capability-disposition-qualification interface-fit screening evidence",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.capability-disposition-qualification-interface-fit-evidence/v1"
            },
            ...
            "selection_policy": {
                "const": "all_48_candidates_from_capability_disposition_qualification_hit_classification"
            },
            "falsifiability_contract": {
                "type": "object",
                "additionalProperties": False,
...
def _build_screening(
    ...
    if (
        classification.get("capability_id") != CAPABILITY_ID
        or classification.get("candidate_count") != EXPECTED_CANDIDATES
        or classification.get("raw_results_digest")
        != search_evidence.get("raw_results", {}).get("digest")
    ):
        raise CapabilityDispositionQualificationSetupError(
            "capability-disposition-qualification classification denominator drifted"
        )
    classification_ids = sorted(
        {
            cast(str, record["candidate_id"])
            for record in cast(list[dict[str, Any]], classification.get("records", []))
            if record.get("classification") == "candidate"
            and isinstance(record.get("candidate_id"), str)
        }
    )
    if len(classification_ids) != EXPECTED_CANDIDATES:
        raise CapabilityDispositionQualificationSetupError(
            "classification does not contain the exact grouped candidate denominator"
        )
    selected = [
        candidate
    ...
    if (
        ...
        or actual_ids != classification_ids
        or len(actual_ids) != EXPECTED_CANDIDATES
    ):
        raise CapabilityDispositionQualificationSetupError(
            "candidate inputs do not equal the committed 48-candidate classification denominator"
        )
    ...
    if memberships != EXPECTED_MEMBERSHIPS or scan_memberships != EXPECTED_MEMBERSHIPS:
        raise CapabilityDispositionQualificationSetupError(
            "capability-disposition-qualification projected membership denominator drifted"
        )
    vector_refs = [
        {
    ...
    document = {
        "schema_version": "pmorg.capability-disposition-qualification-interface-fit-evidence/v1",
        ...
        "contract_test_manifest": _artifact_ref(
            repository_root, CONTRACT_TEST_MANIFEST_RELATIVE
        ),
        ...
        "selection_policy": "all_48_candidates_from_capability_disposition_qualification_hit_classification",
    ...
    if errors:
        raise CapabilityDispositionQualificationSetupError(
            f"capability-disposition-qualification screening schema violation: {errors[0]}"
        )
    return {
...
def build_capability_disposition_qualification_setup(
...
def write_capability_disposition_qualification_setup(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    for relative_path, payload in build_capability_disposition_qualification_setup(
        repository_root
    ).items():
...
def check_capability_disposition_qualification_setup(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    for relative_path, payload in build_capability_disposition_qualification_setup(
        ...
        try:
            ...
            raise CapabilityDispositionQualificationSetupError(
                f"committed Q7a artifact is missing: {relative_path}"
            ) from error
        if actual != payload:
            raise CapabilityDispositionQualificationSetupError(
                f"committed Q7a artifact drifted: {relative_path}"
            )
