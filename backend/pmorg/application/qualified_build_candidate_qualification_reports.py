"""Execute qualified reproducible build qualification over the denominator."""
...
CONTRACT_TEST_RELATIVE = (
    "pmorg/capabilities/contract-tests/qualified-reproducible-build.json"
)
...
INDEX_SCHEMA_RELATIVE = (
    "pmorg/capabilities/qualified-build-candidate-qualification-reports-v1.schema.json"
)
RESULT_ROOT_RELATIVE = (
    "pmorg/capabilities/qualified-build-candidate-qualification-results"
)
REPORT_ROOT_RELATIVE = (
    "pmorg/capabilities/qualified-build-candidate-qualification-reports"
)
DERIVATION_RELATIVES = (
    "backend/pmorg/application/qualified_build_candidate_qualification_reports.py",
    "pmorg/scripts/build_qualified_build_candidate_qualification_reports.py",
)

INDEX_SCHEMA_VERSION = "pmorg.qualified-build-candidate-qualification-report-index/v1"
INDEX_VERSION = "1.0.0"
...
class QualifiedBuildCandidateQualificationReportError(ValueError):
    """Raised when Qualified Build qualification evidence is incomplete or drifted."""
...
def _contract_test_ref(repository_root: Path) -> dict[str, Any]:
    ...
    if item is None or len(cast(list[Any], item.get("contract_tests", []))) != 1:
        raise QualifiedBuildCandidateQualificationReportError(
            "Qualified Build contract-test binding is incomplete"
        )
    ...
    if observed != expected:
        raise QualifiedBuildCandidateQualificationReportError(
            "Qualified Build contract-test binding drifted"
        )
    return observed
...
def _verified_payload(
    ...
    if reference.get("digest") != sha256_digest(payload):
        raise QualifiedBuildCandidateQualificationReportError(f"{label} digest drifted")
    if reference.get("size_bytes") != len(payload):
        raise QualifiedBuildCandidateQualificationReportError(f"{label} size drifted")
    return payload
...
def index_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "$id": "urn:pmorg:qualified-build-candidate-qualification-report-index:v1",
        "title": "PMORG Qualified Build candidate qualification report index",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            ...
            "claim_boundary": {
                "const": "qualified_build_candidate_qualification_only_no_disposition"
            },
        },
    }


def build_qualified_build_candidate_qualification_outputs(
    ...
    if len(candidates) != REPORT_COUNT:
        raise QualifiedBuildCandidateQualificationReportError(
            f"Qualified Build candidate denominator drifted: {len(candidates)}"
        )
    blob_sets = {
    ...
    extension_bytes = build_qualified_build_oracle_extension(repository_root)[
        EXTENSION_RELATIVE
    ]
    if _safe_path(repository_root, EXTENSION_RELATIVE).read_bytes() != extension_bytes:
        raise QualifiedBuildCandidateQualificationReportError(
            "committed Qualified Build oracle extension drifted"
        )
    ...
    if [oracle.get("test_id") for oracle in oracles] != list(TEST_IDS) or any(
        ...
        raise QualifiedBuildCandidateQualificationReportError(
            "Qualified Build oracle extension is not completely executable"
        )
    extension_ref = _artifact_ref(
        repository_root,
        EXTENSION_RELATIVE,
        logical_name="qualification-oracle-extension-qualified-build-v1",
    )
    vector_refs = {
    ...
    for candidate in candidates:
        ...
        for test_id in TEST_IDS:
            ...
            result_ref = _payload_ref(
                result_path,
                result_payload,
                logical_name=f"{candidate_id}-{test_id}-qualified-build-oracle-result",
            )
        ...
        report_ref = _payload_ref(
            report_path,
            report_payload,
            logical_name=f"{candidate_id}-qualified-build-qualification-report",
        )
    ...
    index = {
        ...
        "entries": entries,
        "claim_boundary": "qualified_build_candidate_qualification_only_no_disposition",
    }
    errors = sorted(Draft202012Validator(index_schema()).iter_errors(index), key=str)
    if errors:
        raise QualifiedBuildCandidateQualificationReportError(
            f"Qualified Build qualification index schema violation: {errors[0]}"
        )
    outputs[INDEX_RELATIVE] = canonical_document_bytes(index)
    return outputs


def write_qualified_build_candidate_qualification_reports(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    outputs = build_qualified_build_candidate_qualification_outputs(repository_root)
...
def check_qualified_build_candidate_qualification_reports(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    ...
    if errors:
        raise QualifiedBuildCandidateQualificationReportError(
            f"committed Qualified Build index is invalid: {errors[0]}"
        )
    bundle = _read_object(repository_root, CANDIDATE_INPUTS_RELATIVE)
    validate_candidate_input_bundle(bundle)
    catalog_hash = sha256_digest(
    ...
    expected_extension_bytes = build_qualified_build_oracle_extension(repository_root)[
        EXTENSION_RELATIVE
    ]
    if (
    ...
    if set(oracles) != set(TEST_IDS):
        raise QualifiedBuildCandidateQualificationReportError(
            "committed Qualified Build oracle coverage drifted"
        )
    ...
    if (
        ...
        raise QualifiedBuildCandidateQualificationReportError(
            "committed Qualified Build index binding drifted"
        )
    candidates = {
    ...
    if [item["candidate_id"] for item in entries] != sorted(candidates):
        raise QualifiedBuildCandidateQualificationReportError(
            "committed reports do not exactly cover the Qualified Build denominator"
        )
    ...
    if (
        ...
        raise QualifiedBuildCandidateQualificationReportError("verdict counts drifted")
...
__all__ = [
    "QualifiedBuildCandidateQualificationReportError",
    "build_qualified_build_candidate_qualification_outputs",
