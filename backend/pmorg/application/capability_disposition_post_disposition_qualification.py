"""Build and verify Capability Disposition post-disposition qualification evidence."""
...
QUALIFICATION_RELATIVE = "backend/pmorg/application/qualification.py"
QUALIFICATION_ORACLES_RELATIVE = "backend/pmorg/application/qualification_oracles.py"
TEST_MODULE_RELATIVES = (
    "backend/pmorg/tests/test_qualification_oracles.py",
...
RUNNER_RELATIVE = "pmorg/scripts/run_capability_disposition_post_disposition_suite.py"
BUILD_SCRIPT_RELATIVE = (
    "pmorg/scripts/build_capability_disposition_post_disposition_qualification.py"
)
APPLICATION_RELATIVE = (
    "backend/pmorg/application/capability_disposition_post_disposition_qualification.py"
)
INDEX_RELATIVE = (
    "pmorg/capabilities/capability-disposition-post-disposition-qualification-v1.json"
)
INDEX_SCHEMA_RELATIVE = "pmorg/capabilities/capability-disposition-post-disposition-qualification-v1.schema.json"
MANIFEST_RELATIVE = (
    "pmorg/capabilities/capability-disposition-post-disposition-test-manifest-v1.json"
)
MANIFEST_SCHEMA_RELATIVE = "pmorg/capabilities/capability-disposition-post-disposition-test-manifest-v1.schema.json"
RESULT_SCHEMA_RELATIVE = "pmorg/capabilities/capability-disposition-post-disposition-test-result-v1.schema.json"
SUITE_RECEIPT_RELATIVE = (
    "pmorg/capabilities/capability-disposition-post-disposition-suite-receipt-v1.json"
)
SUITE_RECEIPT_SCHEMA_RELATIVE = "pmorg/capabilities/capability-disposition-post-disposition-suite-receipt-v1.schema.json"
REPORT_RELATIVE = "pmorg/capabilities/capability-disposition-post-disposition-qualification-report-v1.json"
RESULT_ROOT_RELATIVE = (
    "pmorg/capabilities/capability-disposition-post-disposition-test-results"
)
...
PREDECESSOR_EXACT_PATHS = (
    "backend/pmorg/application/capability_disposition_qualification_setup.py",
    "backend/pmorg/application/capability_disposition_interface_fit_executor.py",
    (
        "backend/pmorg/application/"
        "capability_disposition_qualification_candidate_qualification_reports.py"
    ),
    "backend/pmorg/tests/test_capability_disposition_qualification_setup.py",
    "backend/pmorg/tests/test_capability_disposition_interface_fit_executor.py",
    (
        "backend/pmorg/tests/"
        "test_capability_disposition_qualification_candidate_qualification_reports.py"
    ),
    "pmorg/scripts/build_capability_disposition_qualification_setup.py",
    "pmorg/scripts/build_capability_disposition_interface_fit_executor.py",
    (
        "pmorg/scripts/"
        "build_capability_disposition_qualification_candidate_qualification_reports.py"
    ),
    "pmorg/capabilities/capability-disposition-report-v1.json",
)
PREDECESSOR_PATH_PREFIXES = (
    "pmorg/capabilities/dispositions/",
    "pmorg/capabilities/deployment-admission-",
    "pmorg/capabilities/distribution-admission-",
    "pmorg/capabilities/candidate-search/deployment-admission-",
    "pmorg/capabilities/candidate-search/distribution-admission-",
    ...
    "pmorg/capabilities/post-disposition-qualification-reports/distribution-admission.json",
    "pmorg/capabilities/thin-fork-post-disposition-",
    "pmorg/capabilities/governed-fork-post-disposition-",
    "pmorg/capabilities/qualified-build-post-disposition-",
    "pmorg/capabilities/capability-disposition-qualification-",
    (
        "pmorg/capabilities/qualification-test-vector-extension-"
        "capability-disposition-qualification-v1.json"
    ),
    (
    ...
    (
        "pmorg/capabilities/qualification-oracle-extension-"
        "capability-disposition-qualification"
    ),
)
...
ALLOWED_SLICE_PATHS = {
    ...
    "backend/pmorg/tests/test_capability_disposition_post_disposition_qualification.py",
}
...
class CapabilityDispositionPostDispositionQualificationError(ValueError):
    """Raised when Capability Disposition PDQ evidence is incomplete or drifted."""
...
def _safe_path(repository_root: Path, relative_path: str) -> Path:
    ...
    try:
        ...
        raise CapabilityDispositionPostDispositionQualificationError(
...
def _historical_blob(
    ...
    try:
        ...
        raise CapabilityDispositionPostDispositionQualificationError(
            f"Capability Disposition historical path is absent from {commit}: {relative_path}"
        ) from error
...
def _base_predecessor_paths(repository_root: Path) -> tuple[str, ...]:
    try:
        completed = subprocess.run(
            ["git", "ls-tree", "-r", "--name-only", BASE_PLATFORM_COMMIT],
            cwd=repository_root,
            check=True,
            capture_output=True,
            text=True,
        ...
        raise CapabilityDispositionPostDispositionQualificationError(
            "cannot enumerate Q7d predecessor paths"
        ) from error
    tracked = completed.stdout.splitlines()
    selected = {
        path
        for path in tracked
        if path in PREDECESSOR_EXACT_PATHS
        or any(path.startswith(prefix) for prefix in PREDECESSOR_PATH_PREFIXES)
    }
    missing = set(PREDECESSOR_EXACT_PATHS) - selected
    if missing:
        raise CapabilityDispositionPostDispositionQualificationError(
            f"Q7d predecessor path set is incomplete: {sorted(missing)}"
        )
    return tuple(sorted(selected))


def _assert_predecessors_immutable(repository_root: Path) -> None:
    for relative_path in _base_predecessor_paths(repository_root):
        try:
            live = _safe_path(repository_root, relative_path).read_bytes()
        except OSError as error:
            raise CapabilityDispositionPostDispositionQualificationError(
                f"Q7d predecessor is missing: {relative_path}"
            ) from error
        if live != _git_blob(repository_root, relative_path):
            raise CapabilityDispositionPostDispositionQualificationError(
                f"Q7d predecessor drifted: {relative_path}"
            )
    ...
    for relative_path in (
        ...
        *TEST_MODULE_RELATIVES,
        ...
        if _safe_path(repository_root, relative_path).read_bytes() != _git_blob(
            ...
            raise CapabilityDispositionPostDispositionQualificationError(
                f"trusted Capability Disposition path changed in PDQ slice: {relative_path}"
            )
...
def _historical_result_paths(repository_root: Path) -> set[str]:
    ...
    try:
        ...
        raise CapabilityDispositionPostDispositionQualificationError(
            "cannot enumerate historical Q7d result paths"
        ) from error
...
def _assert_archival_evidence_immutable(repository_root: Path) -> None:
    ...
    for relative_path in sorted(historical_paths):
        try:
            ...
            raise CapabilityDispositionPostDispositionQualificationError(
                f"Q7d archival evidence is missing: {relative_path}"
            ) from error
        historical = _historical_blob(
            repository_root, relative_path, commit=Q7D_EVIDENCE_COMMIT
        )
        if live != historical:
            raise CapabilityDispositionPostDispositionQualificationError(
                f"Q7d archival evidence drifted: {relative_path}"
            )
...
def _assert_append_only_sequence(*, base: Any, live: Any, label: str) -> None:
    if not isinstance(base, list) or not isinstance(live, list):
        raise CapabilityDispositionPostDispositionQualificationError(
            f"Q7d patch ledger {label} is not a list"
        )
    if len(live) < len(base) or live[: len(base)] != base:
        raise CapabilityDispositionPostDispositionQualificationError(
            f"Q7d patch ledger {label} history is not an exact prefix"
        )
    identifiers: list[str] = []
    for item in live:
        if not isinstance(item, dict) or not isinstance(item.get("id"), str):
            raise CapabilityDispositionPostDispositionQualificationError(
                f"Q7d patch ledger {label} contains an invalid ID"
            )
        identifiers.append(cast(str, item["id"]))
    if len(identifiers) != len(set(identifiers)):
        raise CapabilityDispositionPostDispositionQualificationError(
            f"Q7d patch ledger {label} IDs are not unique"
        )
...
def _assert_patch_ledger_append_only(repository_root: Path) -> None:
    try:
        ...
        raise CapabilityDispositionPostDispositionQualificationError(
            "Q7d patch ledger cannot be read as JSON"
        ) from error
    if not isinstance(base, dict) or not isinstance(live, dict):
        raise CapabilityDispositionPostDispositionQualificationError(
            "Q7d patch ledger is not an object"
        )
    if set(live) != set(base):
        raise CapabilityDispositionPostDispositionQualificationError(
            "Q7d patch ledger top-level structure drifted"
        )
    for key in LEDGER_TOP_LEVEL_PINS:
        if live.get(key) != base.get(key):
            raise CapabilityDispositionPostDispositionQualificationError(
                f"Q7d patch ledger top-level pin drifted: {key}"
            )
...
def _assert_historical_integrity(repository_root: Path) -> None:
    """Bind predecessor evidence, Q7d evidence, and append-only registries."""
...
def _ledger_entries(repository_root: Path) -> list[dict[str, Any]]:
    ...
    if any(item not in by_id for item in LEDGER_IDS):
        raise CapabilityDispositionPostDispositionQualificationError(
            "Capability Disposition patch-ledger ownership is missing"
        )
...
def _run_runner(
    ...
    if list_only:
        ...
        if not isinstance(value, list) or not all(
            ...
            raise CapabilityDispositionPostDispositionQualificationError(
                "Capability Disposition runner emitted an invalid test list"
            )
    ...
    if not isinstance(value, dict):
        raise CapabilityDispositionPostDispositionQualificationError(
            "Capability Disposition runner emitted an invalid suite receipt"
        )
...
def _manifest_document(
    ...
    implementation_bindings = [
        _binding(repository_root, QUALIFICATION_RELATIVE),
        _binding(repository_root, QUALIFICATION_ORACLES_RELATIVE),
    ]
    test_module_bindings = [
        _binding(repository_root, relative_path)
        for relative_path in TEST_MODULE_RELATIVES
    ]
    ledger_entries = _ledger_entries(repository_root)
    return {
        "schema_version": "pmorg.capability-disposition-post-disposition-test-manifest/v1",
        ...
        "test_module_bindings": test_module_bindings,
        ...
        "execution": {
            ...
            "mode": "single-process-complete-modules",
            "suites": list(TEST_SUITE_NAMES),
        },
        "test_cases": [
            {
...
def manifest_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "$id": "urn:pmorg:capability-disposition-post-disposition-test-manifest:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.capability-disposition-post-disposition-test-manifest/v1"
            },
            ...
            "execution": {
                "type": "object",
                "additionalProperties": False,
                "required": ["framework", "mode", "suites"],
                "properties": {
                    "framework": {"const": "python-unittest"},
                    "mode": {"const": "single-process-complete-modules"},
                    "suites": {"const": list(TEST_SUITE_NAMES)},
                },
            },
            "test_cases": {
                "type": "array",
...
def suite_receipt_schema() -> dict[str, Any]:
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "urn:pmorg:capability-disposition-post-disposition-suite-receipt:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.capability-disposition-post-disposition-suite-receipt/v1"
            },
            "pmorg_platform_commit": {"const": BASE_PLATFORM_COMMIT},
            "suites": {"const": list(TEST_SUITE_NAMES)},
...
def result_schema() -> dict[str, Any]:
    ...
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "urn:pmorg:capability-disposition-post-disposition-test-result:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.capability-disposition-post-disposition-test-result/v1"
            },
            "capability_id": {"const": CAPABILITY_ID},
            "test_id": {"type": "string", "minLength": 1},
            "fully_qualified_name": {"type": "string", "minLength": 1},
...
def index_schema() -> dict[str, Any]:
    ...
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "urn:pmorg:capability-disposition-post-disposition-qualification-index:v1",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            "schema_version": {
                "const": "pmorg.capability-disposition-post-disposition-qualification-index/v1"
            },
...
def build_capability_disposition_post_disposition_qualification(
    ...
    if len(identities) != TEST_COUNT:
        raise CapabilityDispositionPostDispositionQualificationError(
            "Capability Disposition PDQ test denominator drifted"
        )
    ...
    for item in cast(list[dict[str, Any]], receipt["tests"]):
        ...
        test_id = _test_id(identity)
        result = {
            "schema_version": "pmorg.capability-disposition-post-disposition-test-result/v1",
            "capability_id": CAPABILITY_ID,
            "test_id": test_id,
            "fully_qualified_name": identity,
    ...
    try:
        ...
        raise CapabilityDispositionPostDispositionQualificationError(
            "Capability Disposition PDQ report failed the contract model"
        ) from error
    ...
    if errors:
        raise CapabilityDispositionPostDispositionQualificationError(
            f"Capability Disposition PDQ report schema failed: {errors[0]}"
        )
    ...
    report_ref = _payload_ref(
        REPORT_RELATIVE,
        report_payload,
        logical_name="capability-disposition-post-disposition-qualification-report",
    )
    derivation_refs = [
        _artifact_ref(
            repository_root,
    ...
    index = {
        "schema_version": (
            "pmorg.capability-disposition-post-disposition-qualification-index/v1"
        ),
    ...
    if errors:
        raise CapabilityDispositionPostDispositionQualificationError(
            f"Capability Disposition PDQ index schema failed: {errors[0]}"
        )
    outputs[INDEX_RELATIVE] = canonical_document_bytes(index)
    return outputs


def write_capability_disposition_post_disposition_qualification(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    outputs = build_capability_disposition_post_disposition_qualification(
        repository_root
    )
...
def check_capability_disposition_post_disposition_qualification(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    _assert_trust_boundary_immutable(repository_root)
    ...
    if manifest != _manifest_document(repository_root, identities):
        raise CapabilityDispositionPostDispositionQualificationError(
            "Capability Disposition PDQ manifest derivation drifted"
        )
    receipt = json.loads(
        _verified_payload(
    ...
    if [item["fully_qualified_name"] for item in receipt["tests"]] != identities:
        raise CapabilityDispositionPostDispositionQualificationError(
            "Capability Disposition PDQ receipt denominator drifted"
        )
    runtime_measurement, runtime_digest = _runtime_measurement(repository_root, receipt)
    if index["runtime_identity_digests"] != [runtime_digest]:
        raise CapabilityDispositionPostDispositionQualificationError(
            "Capability Disposition PDQ runtime identity drifted"
        )
    ...
    try:
        ...
        raise CapabilityDispositionPostDispositionQualificationError(
            "committed Capability Disposition PDQ report failed the contract model"
        ) from error
    ...
    if len(result_refs) != TEST_COUNT or len(evidence) != TEST_COUNT:
        raise CapabilityDispositionPostDispositionQualificationError(
            "Capability Disposition PDQ result coverage drifted"
        )
    ...
    for identity, reference, item in zip(
        ...
        if (
            ...
            raise CapabilityDispositionPostDispositionQualificationError(
                f"Capability Disposition PDQ evidence drifted: {identity}"
            )
    ...
    if (
        ...
        raise CapabilityDispositionPostDispositionQualificationError(
            "Capability Disposition PDQ report counters or bindings drifted"
        )
    committed_paths = {
    ...
    if committed_paths != observed_paths:
        raise CapabilityDispositionPostDispositionQualificationError(
            "Capability Disposition PDQ result directory is not byte-closed"
        )
    for reference in cast(list[dict[str, Any]], index["derivation_artifacts"]):
        ...
        if relative_path == APPLICATION_RELATIVE:
            ...
            payload = _historical_blob(
                repository_root,
                relative_path,
                commit=Q7D_EVIDENCE_COMMIT,
            )
            if reference.get("digest") != sha256_digest(payload) or reference.get(
                "size_bytes"
            ) != len(payload):
                raise CapabilityDispositionPostDispositionQualificationError(
                    "historical Q7d derivation reference drifted"
                )
        else:
            _verified_payload(
                repository_root, reference, label="PDQ derivation artifact"
...
__all__ = [
    ...
    "build_capability_disposition_post_disposition_qualification",
