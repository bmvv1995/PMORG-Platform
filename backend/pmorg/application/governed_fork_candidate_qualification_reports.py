"""Execute governed Onyx fork qualification over the complete denominator."""
...
CONTRACT_TEST_RELATIVE = "pmorg/capabilities/contract-tests/governed-onyx-fork.json"
...
INDEX_SCHEMA_RELATIVE = (
    "pmorg/capabilities/governed-fork-candidate-qualification-reports-v1.schema.json"
)
RESULT_ROOT_RELATIVE = (
    "pmorg/capabilities/governed-fork-candidate-qualification-results"
)
REPORT_ROOT_RELATIVE = (
    "pmorg/capabilities/governed-fork-candidate-qualification-reports"
)
DERIVATION_RELATIVES = (
    "backend/pmorg/application/governed_fork_candidate_qualification_reports.py",
    "pmorg/scripts/build_governed_fork_candidate_qualification_reports.py",
)

INDEX_SCHEMA_VERSION = "pmorg.governed-fork-candidate-qualification-report-index/v1"
INDEX_VERSION = "1.0.0"
...
class GovernedForkCandidateQualificationReportError(ValueError):
    """Raised when Governed Fork qualification evidence is incomplete or drifted."""
...
def _contract_test_ref(repository_root: Path) -> dict[str, Any]:
    ...
    if item is None or len(cast(list[Any], item.get("contract_tests", []))) != 1:
        raise GovernedForkCandidateQualificationReportError(
            "Governed Fork contract-test binding is incomplete"
        )
    ...
    if observed != expected:
        raise GovernedForkCandidateQualificationReportError(
            "Governed Fork contract-test binding drifted"
        )
    return observed
...
def _verified_payload(
    ...
    if reference.get("digest") != sha256_digest(payload):
        raise GovernedForkCandidateQualificationReportError(f"{label} digest drifted")
    if reference.get("size_bytes") != len(payload):
        raise GovernedForkCandidateQualificationReportError(f"{label} size drifted")
    return payload
...
def index_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "$id": "urn:pmorg:governed-fork-candidate-qualification-report-index:v1",
        "title": "PMORG Governed Fork candidate qualification report index",
        "type": "object",
        "additionalProperties": False,
        "required": [
            "schema_version",
        ...
        "properties": {
            ...
            "claim_boundary": {
                "const": "governed_fork_candidate_qualification_only_no_disposition"
            },
        },
    }


def build_governed_fork_candidate_qualification_outputs(
    ...
    if len(candidates) != REPORT_COUNT:
        raise GovernedForkCandidateQualificationReportError(
            f"Governed Fork candidate denominator drifted: {len(candidates)}"
        )
    blob_sets = {
    ...
    extension_bytes = build_governed_fork_oracle_extension(repository_root)[
        EXTENSION_RELATIVE
    ]
    if _safe_path(repository_root, EXTENSION_RELATIVE).read_bytes() != extension_bytes:
        raise GovernedForkCandidateQualificationReportError(
            "committed Governed Fork oracle extension drifted"
        )
    ...
    if [oracle.get("test_id") for oracle in oracles] != list(TEST_IDS) or any(
        ...
        raise GovernedForkCandidateQualificationReportError(
            "Governed Fork oracle extension is not completely executable"
        )
    extension_ref = _artifact_ref(
        repository_root,
        EXTENSION_RELATIVE,
        logical_name="qualification-oracle-extension-governed-fork-v1",
    )
    vector_refs = {
    ...
    for candidate in candidates:
        ...
        for test_id in TEST_IDS:
            ...
            result_ref = _payload_ref(
                result_path,
                result_payload,
                logical_name=f"{candidate_id}-{test_id}-governed-fork-oracle-result",
            )
        ...
        report_ref = _payload_ref(
            report_path,
            report_payload,
            logical_name=f"{candidate_id}-governed-fork-qualification-report",
        )
    ...
    index = {
        ...
        "entries": entries,
        "claim_boundary": "governed_fork_candidate_qualification_only_no_disposition",
    }
    errors = sorted(Draft202012Validator(index_schema()).iter_errors(index), key=str)
    if errors:
        raise GovernedForkCandidateQualificationReportError(
            f"Governed Fork qualification index schema violation: {errors[0]}"
        )
    outputs[INDEX_RELATIVE] = canonical_document_bytes(index)
    return outputs


def write_governed_fork_candidate_qualification_reports(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    outputs = build_governed_fork_candidate_qualification_outputs(repository_root)
...
def check_governed_fork_candidate_qualification_reports(
    repository_root: Path = REPOSITORY_ROOT,
) -> None:
    repository_root = repository_root.resolve()
    ...
    if errors:
        raise GovernedForkCandidateQualificationReportError(
            f"committed Governed Fork index is invalid: {errors[0]}"
        )
    bundle = _read_object(repository_root, CANDIDATE_INPUTS_RELATIVE)
    validate_candidate_input_bundle(bundle)
    catalog_hash = sha256_digest(
    ...
    expected_extension_bytes = build_governed_fork_oracle_extension(repository_root)[
        EXTENSION_RELATIVE
    ]
    if (
    ...
    if set(oracles) != set(TEST_IDS):
        raise GovernedForkCandidateQualificationReportError(
            "committed Governed Fork oracle coverage drifted"
        )
    ...
    if (
        ...
        raise GovernedForkCandidateQualificationReportError(
            "committed Governed Fork index binding drifted"
        )
    candidates = {
    ...
    if [item["candidate_id"] for item in entries] != sorted(candidates):
        raise GovernedForkCandidateQualificationReportError(
            "committed reports do not exactly cover the Governed Fork denominator"
        )
    ...
    for entry in entries:
        ...
        if list(report_validator.iter_errors(report)):
            raise GovernedForkCandidateQualificationReportError("report schema drifted")
        try:
            CandidateQualificationReport.model_validate(report)
        except ValidationError as error:
            raise GovernedForkCandidateQualificationReportError(
    ...
    if (
        ...
        raise GovernedForkCandidateQualificationReportError("verdict counts drifted")
    if sorted(runtime_digests) != index["runtime_identity_digests"]:
        raise GovernedForkCandidateQualificationReportError("runtime identity drifted")
