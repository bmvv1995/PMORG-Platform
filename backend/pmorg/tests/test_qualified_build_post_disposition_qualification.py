...
from pmorg.application.qualified_build_post_disposition_qualification import (
...
from pmorg.application.qualified_build_post_disposition_qualification import TEST_COUNT
from pmorg.application.qualified_build_post_disposition_qualification import (
...
class QualifiedBuildPostDispositionQualificationTests(unittest.TestCase):
    ...
    def test_committed_evidence_graph_is_closed_and_valid(self) -> None:
        check_qualified_build_post_disposition_qualification(REPOSITORY_ROOT)
        ...
        self.assertEqual(
            "qualified_build_post_disposition_qualification_only_no_disposition",
            index["claim_boundary"],
        )
        report = json.loads((REPOSITORY_ROOT / REPORT_RELATIVE).read_bytes())
        self.assertEqual("qualified-reproducible-build", report["capability_id"])
    ...
    def test_manifest_projects_exact_bounded_suite_and_trust_paths(self) -> None:
        ...
        self.assertEqual(
            [BUILD_CE_ARTIFACT_RELATIVE, VERIFY_CE_BUILD_RELATIVE],
    ...
    def test_benign_successor_and_unique_ledger_append_pass(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            appended["paths"] = ["backend/pmorg/application/future_q6_slice.py"]
            appended["reason"] = "Benign successor-safety simulation."
            ledger["entries"].append(appended)
            ledger_path.write_text(json.dumps(ledger), encoding="utf-8")
            check_qualified_build_post_disposition_qualification(copied)
    ...
    def test_static_trust_file_tamper_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            copied = _copy_repository(temporary_directory)
            path = copied / BUILD_CE_ARTIFACT_RELATIVE
            path.write_bytes(path.read_bytes() + b"\n# tamper\n")
            with self.assertRaisesRegex(
                QualifiedBuildPostDispositionQualificationError,
                "trusted Qualified Build path changed",
            ):
                check_qualified_build_post_disposition_qualification(copied)
