...
class QualifiedBuildCandidateQualificationReportTests(unittest.TestCase):
    def test_committed_index_is_closed_complete_and_schema_valid(self) -> None:
        check_qualified_build_candidate_qualification_reports(REPOSITORY_ROOT)
        index = json.loads((REPOSITORY_ROOT / INDEX_RELATIVE).read_bytes())
        Draft202012Validator(index_schema()).validate(index)
        self.assertEqual(46, index["report_count"])
        self.assertEqual(184, index["executed_test_count"])
        self.assertEqual(0, index["missing_test_count"])
        self.assertEqual(0, index["duplicate_test_count"])
        self.assertEqual(
            "qualified_build_candidate_qualification_only_no_disposition",
            index["claim_boundary"],
        )

    def test_every_candidate_has_four_bound_results_and_one_report(self) -> None:
        index = json.loads((REPOSITORY_ROOT / INDEX_RELATIVE).read_bytes())
        identities = [entry["candidate_id"] for entry in index["entries"]]
        self.assertEqual(identities, sorted(set(identities)))
        self.assertEqual(46, len(identities))
        for entry in index["entries"]:
            report = json.loads(
                (REPOSITORY_ROOT / entry["report"]["relative_path"]).read_bytes()
            )
            self.assertEqual(4, report["executed_test_count"])
            self.assertEqual(4, len(entry["results"]))
            self.assertEqual(
    ...
    def test_complete_execution_is_deterministic_within_one_runtime(self) -> None:
        first = build_qualified_build_candidate_qualification_outputs(REPOSITORY_ROOT)
        second = build_qualified_build_candidate_qualification_outputs(REPOSITORY_ROOT)
        self.assertEqual(first, second)
        self.assertEqual(232, len(first))
    ...
    def test_tampered_result_is_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(
                QualifiedBuildCandidateQualificationReportError,
                "candidate result A-EVIDENCE-001 digest drifted",
            ):
                check_qualified_build_candidate_qualification_reports(root)
