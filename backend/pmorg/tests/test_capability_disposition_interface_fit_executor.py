...
from pmorg.application.capability_disposition_interface_fit_executor import _read_blobs
...
class CapabilityDispositionInterfaceFitExecutorTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.candidate_id = next(
            ...
            if item["capability_id"] == "capability-disposition-qualification"
        )

    def test_extension_is_deterministic_schema_valid_and_predecessor_safe(self) -> None:
        first = build_capability_disposition_oracle_extension(REPOSITORY_ROOT)
        second = build_capability_disposition_oracle_extension(REPOSITORY_ROOT)
        self.assertEqual(first, second)
        document = json.loads(
            first[
                "pmorg/capabilities/qualification-oracle-extension-capability-disposition-qualification-v1.json"
            ]
        )
        ...
        self.assertEqual(
            48, document["candidate_denominator"]["grouped_candidate_count"]
        )
        self.assertEqual(
            5015, document["candidate_denominator"]["blob_membership_count"]
        )

    def test_all_five_executors_consume_candidate_bytes_and_live_mutation(
        self,
    ) -> None:
    ...
    def test_candidate_blob_policy_and_unknown_test_tamper_fail_closed(self) -> None:
        ...
        with patch(
            "pmorg.application.capability_disposition_interface_fit_executor._read_blobs",
            side_effect=mutated,
        ):
        ...
        vector_path = (
            REPOSITORY_ROOT
            / "pmorg/capabilities/qualification-test-vector-extension-capability-disposition-qualification-v1.json"
        )
        ...
        original_read = __import__(
            "pmorg.application.capability_disposition_interface_fit_executor",
        ...
        with patch(
            "pmorg.application.capability_disposition_interface_fit_executor._read_object",
        ...
        with self.assertRaisesRegex(
            CapabilityDispositionInterfaceFitExecutorError, "unknown test id"
        ):
            execute_capability_disposition_interface_fit(
                self.candidate_id, "UNKNOWN", repository_root=REPOSITORY_ROOT
            )

    def test_exact_q7a_denominator_drift_fails_closed(self) -> None:
        with patch(
            "pmorg.application.capability_disposition_interface_fit_executor.EXPECTED_GROUPED_CANDIDATES",
            47,
        ):
            with self.assertRaisesRegex(
                CapabilityDispositionInterfaceFitExecutorError,
    ...
    def test_candidate_inputs_and_q7a_evidence_tamper_fail_closed(self) -> None:
        module = __import__(
            "pmorg.application.capability_disposition_interface_fit_executor",
            fromlist=["_artifact_ref", "_read_object"],
        )
        original_artifact_ref = module._artifact_ref

        def drifted_artifact_ref(
            root: Path, relative_path: str, media_type: str
        ) -> dict[str, object]:
            reference = original_artifact_ref(root, relative_path, media_type)
            if relative_path == "pmorg/capabilities/candidate-inputs-v1.json":
                reference = dict(reference)
                reference["digest"] = "sha256:" + "0" * 64
            return reference

        with patch(
            "pmorg.application.capability_disposition_interface_fit_executor._artifact_ref",
            side_effect=drifted_artifact_ref,
        ):
            with self.assertRaisesRegex(
                CapabilityDispositionInterfaceFitExecutorError,
        ...
        evidence_path = (
            REPOSITORY_ROOT
            / "pmorg/capabilities/capability-disposition-qualification-interface-fit-evidence-v1.json"
        )
        drifted_evidence = json.loads(evidence_path.read_bytes())
        drifted_evidence["candidate_count"] = 47
        original_read = module._read_object

        def drifted_read(root: Path, relative_path: str) -> dict[str, object]:
            if relative_path.endswith(
                "capability-disposition-qualification-interface-fit-evidence-v1.json"
            ):
                return drifted_evidence
            return original_read(root, relative_path)
    ...
    def test_no_op_mutation_is_rejected(self) -> None:
        with patch(
            "pmorg.application.capability_disposition_interface_fit_executor._mutated_blobs",
    ...
    def test_committed_extension_matches_generation(self) -> None:
        check_capability_disposition_oracle_extension(REPOSITORY_ROOT)
