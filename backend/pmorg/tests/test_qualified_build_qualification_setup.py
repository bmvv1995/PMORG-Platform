...
from pmorg.application.qualified_build_qualification_setup import _read_blobs
...
from pmorg.application.qualified_build_qualification_setup import evidence_schema
from pmorg.application.qualified_build_qualification_setup import (
    QualifiedBuildQualificationSetupError,
)

REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
EVIDENCE_PATH = "pmorg/capabilities/qualified-build-interface-fit-evidence-v1.json"
MANIFEST_PATH = (
...
class QualifiedBuildQualificationSetupTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.outputs = build_qualified_build_qualification_setup(REPOSITORY_ROOT)
    ...
    def test_generation_is_deterministic_and_schema_valid(self) -> None:
        self.assertEqual(
            self.outputs,
            build_qualified_build_qualification_setup(REPOSITORY_ROOT),
        )
    ...
    def test_exact_four_vectors_are_unactivated_and_falsifiable(self) -> None:
        self.assertEqual(
            TEST_IDS,
    ...
    def test_exact_committed_denominator_is_screened_byte_closed(self) -> None:
        self.assertEqual(46, self.evidence["candidate_count"])
        self.assertEqual(46, self.evidence["blob_set_scan_count"])
        self.assertEqual(4522, self.evidence["candidate_blob_membership_count"])
        self.assertEqual(4522, self.evidence["blob_set_membership_count"])
        self.assertEqual(
            {"expected": 4522, "scanned": 4522, "unreadable": 0, "unverified": 0},
    ...
    def test_blob_mutation_fails_closed_before_evidence(self) -> None:
        ...
        with patch(
            "pmorg.application.qualified_build_qualification_setup._read_blobs",
            side_effect=mutated,
        ):
    ...
    def test_committed_outputs_match_and_predecessors_are_not_outputs(self) -> None:
        check_qualified_build_qualification_setup(REPOSITORY_ROOT)
        self.assertNotIn(
