#!/usr/bin/env python3
"""Execute the exact Qualified Build verifier test module and emit a closed receipt."""
...
def _load_suite() -> tuple[unittest.TestSuite, list[str]]:
    ...
    if len(identities) != EXPECTED_TEST_COUNT:
        raise RuntimeError(
            f"Qualified Build verifier suite drifted: {len(identities)} tests"
        )
    if identities != sorted(set(identities)):
        raise RuntimeError(
            "Qualified Build verifier suite identities are not unique/sorted"
        )
...
def _receipt() -> dict[str, Any]:
    ...
    if result.testsRun != EXPECTED_TEST_COUNT or any(
        ...
        raise RuntimeError("Qualified Build verifier suites did not pass exactly 12/12")
    interpreter = Path(sys.executable).resolve()
    return {
        "schema_version": "pmorg.qualified-build-post-disposition-suite-receipt/v1",
