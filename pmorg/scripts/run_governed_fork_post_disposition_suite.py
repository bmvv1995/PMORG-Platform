#!/usr/bin/env python3
"""Execute the exact Governed Fork verifier test module and emit a closed receipt."""
...
def _load_suite() -> tuple[unittest.TestSuite, list[str]]:
    ...
    if len(identities) != EXPECTED_TEST_COUNT:
        raise RuntimeError(
            f"Governed Fork verifier suite drifted: {len(identities)} tests"
        )
    if identities != sorted(set(identities)):
        raise RuntimeError(
            "Governed Fork verifier suite identities are not unique/sorted"
        )
...
def _receipt() -> dict[str, Any]:
    ...
    if result.testsRun != EXPECTED_TEST_COUNT or any(
        ...
        raise RuntimeError("Governed Fork verifier suite did not pass exactly 87/87")
    interpreter = Path(sys.executable).resolve()
    return {
        "schema_version": "pmorg.governed-fork-post-disposition-suite-receipt/v1",
